...
`scripts/sync_to_github.py` 面向本地项目目录执行以下流程：
...
```bash
# 默认同步 main，使用单行说明
python scripts/sync_to_github.py -m "Add exchange accounting tests"

# 同步其他 branch，并提供多行 merge message
python scripts/sync_to_github.py -b research --comment-file /tmp/research-message.txt

# 只验证本地提交/合并，不上传
python scripts/sync_to_github.py --no-push -b main -m "Dry-run sync"
```
...
用户自行移除；脚本不会自动删除文件。merge 冲突也会保留在工作区，解决后重新运行。
