...
def evaluate(config, directory, destination):
    ...
    report = [
        "# Meta Compute 2026：真实数据与 LLM 初轮实验报告",
        "",
        "本报告由归档数据、逐单日志和已完成的运行自动生成。规则基线、mock 与真实 API 分目录保存。",
        "## 数据与事件时钟",
        "",
    ...
    report += [
        ...
        "## API 与复现",
        "",
        f"模型：{config['llm']['model']}；temperature={config['llm']['temperature']}，max_tokens={config['llm']['max_tokens']}，thinking 关闭，全局并发上限 {config['llm']['max_concurrency']}。",
        f"实际 HTTP 调用 {usage['calls']} 次，成功 {usage['successful_http_calls']} 次，失败 {usage['failed_calls']} 次；本地缓存命中 {usage['local_cache_hits']} 次。输入 {usage['input_tokens']:,} tokens，输出 {usage['output_tokens']:,} tokens。",
        f"按调用时段与可见缓存 token 估算费用 ¥{usage['estimated_cost_cny']:.4f}，不等同到账单实扣。[定价公告](https://docs.siliconflow.cn/docs/release-notes/overview)。",
        "单 agent 预检也计入 API 用量，但不混入 30-agent 主实验统计。请求/响应、trace、usage、配置、源码 hash、Git commit、数据 hash 及依赖版本均保存在 outputs 下；原始响应存 data/raw。API key 仅存在本地忽略文件 .env。",
        "## 结论边界与后续实验",
        "",
